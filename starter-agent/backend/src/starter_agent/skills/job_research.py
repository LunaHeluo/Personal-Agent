...
from starter_agent.job_research.validation import JobValidation, validate_job
...
class JobResearchOrchestrator:
    ...
    async def analyze_candidates(
        ...
        *,
        ...
        for index, candidate in enumerate(candidates[:max_candidates]):
            ...
            job = self._job_payload(snapshot_result.data)
            job = self._merge_candidate_company(job, candidate)
    ...
    @staticmethod
    def _merge_candidate_company(
        job: dict[str, Any],
        candidate: JobCandidate,
    ) -> dict[str, Any]:
        page_company = str(job.get("company") or "").strip()
        page_attribution = {
            "company": page_company,
            "company_source": (
                str(job.get("company_source") or "page_html")
                if page_company
                else ""
            ),
            "company_confidence": (
                str(job.get("company_confidence") or "high")
                if page_company
                else ""
            ),
        }
    ...
    def _validate_job(
        ...
        return validate_job(job, selected_url)
