...
class ModelPricingTier(BaseModel):
    """A configured billing tier expressed in currency microunits per token."""

    max_input_tokens: int | None = Field(default=None, ge=1)
    max_output_tokens: int | None = Field(default=None, ge=0)
    input_microunits_per_token: int = Field(ge=0)
    output_microunits_per_token: int = Field(ge=0)


class ModelPricingConfig(BaseModel):
    currency: str = Field(min_length=3, max_length=3, pattern=r"^[A-Z]{3}$")
    price_version: str = Field(min_length=1, max_length=120)
    source_url: str = Field(min_length=1, max_length=500, pattern=r"^https://")
    tiers: list[ModelPricingTier] = Field(min_length=1)
...
class ModelRouteProfile(BaseModel):
    """Non-secret routing metadata for one configured provider/model pair."""

    provider: str = Field(min_length=1, max_length=120)
    model: str = Field(min_length=1, max_length=200)
    purposes: list[
        Literal["router", "planner", "executor", "judge", "recovery"]
    ] = Field(min_length=1)
    capabilities: list[str] = Field(default_factory=list, max_length=64)
    complexities: list[Literal["trivial", "bounded", "complex"]] = Field(
        min_length=1
    )
    ...
    def normalize_unique_values(self) -> "ModelRouteProfile":
        if len(set(self.purposes)) != len(self.purposes):
            raise ValueError("model routing purposes must be unique")
        if len(set(self.capabilities)) != len(self.capabilities):
            raise ValueError("model routing capabilities must be unique")
        if len(set(self.complexities)) != len(self.complexities):
            raise ValueError("model routing complexities must be unique")
        return self


class ModelRoutingConfig(BaseModel):
    config_revision: str = Field(default="1", min_length=1, max_length=120)
    profiles: list[ModelRouteProfile] = Field(default_factory=list, max_length=256)


class RuntimeConfig(BaseModel):
    # Per-run defaults remain small; delegated Specialist contracts enforce
    # their own lower bounds (for example 30 web-navigation steps).
    max_model_calls: int = Field(default=4, ge=1, le=1_000)
    max_tool_calls: int = Field(default=4, ge=0, le=1_000)
...
class JobDocumentParsingConfig(BaseModel):
    """Optional external OCR configuration for uploaded JD screenshots/PDFs."""

    mineru_token_env: str | None = None
    mineru_base_url: str = "https://mineru.net"
    timeout_seconds: int = Field(default=120, ge=10, le=300)


class LegacyJobResearchMigrationConfig(BaseModel):
    enabled: bool = False
    enabled_at: datetime | None = None
    release_window_ends: tuple[datetime, ...] = Field(default_factory=tuple)

    @model_validator(mode="after")
    def validate_enabled_at(self) -> "LegacyJobResearchMigrationConfig":
        if self.enabled and self.enabled_at is None:
            raise ValueError("legacy migration enabled_at is required when enabled")
        return self


class DelegationReleaseConfig(BaseModel):
    """Fail-closed pointer to a persisted Task20 release decision."""

    decision_id: str | None = None
    baseline_report_hash: str | None = None
    candidate_report_hash: str | None = None
...
class AgentSettings(BaseModel):
    ...
    job_document_parsing: JobDocumentParsingConfig = Field(
        default_factory=JobDocumentParsingConfig
    )
    legacy_job_research_migration: LegacyJobResearchMigrationConfig = Field(
        default_factory=LegacyJobResearchMigrationConfig
    )
    delegation_release: DelegationReleaseConfig = Field(
        default_factory=DelegationReleaseConfig
    )
    ...
    def environment_value(self, name: str | None) -> str | None:
        return self._environment_value(name) if name else None
