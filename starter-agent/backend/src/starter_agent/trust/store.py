...
class DelegationReleaseDecisionRow(TrustBase):
    """Durable, report-hash-bound permission to enable the candidate route."""

    __tablename__ = "trust_delegation_release_decisions"

    id: Mapped[str] = mapped_column(String(160), primary_key=True)
    baseline_run_id: Mapped[str] = mapped_column(String(160), index=True)
    candidate_run_id: Mapped[str] = mapped_column(String(160), index=True)
    status: Mapped[str] = mapped_column(String(40), index=True)
    baseline_report_hash: Mapped[str] = mapped_column(String(64), index=True)
    candidate_report_hash: Mapped[str] = mapped_column(String(64), index=True)
...
class TrustTraceEventRow(TrustBase):
    ...
    access_level: Mapped[str | None] = mapped_column(String(80), nullable=True, index=True)
...
class TracePage:
    """Stable cursor page for Trust traces without a second trace store."""

    items: tuple[TrustTraceEvent, ...]
...
class TrustStore:
    ...
    def _migrate_trace_columns(self) -> None:
        ...
        columns = {item["name"] for item in inspect(self.engine).get_columns("trust_trace_events")}
        additions = {"parent_run_id": "VARCHAR(160)", "child_task_id": "VARCHAR(160)", "principal": "VARCHAR(200)", "access_level": "VARCHAR(80)"}
        with self.engine.begin() as connection:
            for name, definition in additions.items():
                if name not in columns:
                    connection.execute(text(f"ALTER TABLE trust_trace_events ADD COLUMN {name} {definition}"))
    ...
    def save_delegation_release_decision(
        self,
        *,
        decision_id: str,
        ...
        row = DelegationReleaseDecisionRow(
            id=decision_id,
            baseline_run_id=str(decision["baseline_run_id"]),
            candidate_run_id=str(decision["candidate_run_id"]),
            status=str(decision["status"]),
            baseline_report_hash=str(decision["baseline_report_hash"]),
            candidate_report_hash=str(decision["candidate_report_hash"]),
        ...
        with Session(self.engine) as db:
            existing = db.get(DelegationReleaseDecisionRow, decision_id)
            if existing is not None:
                if existing.payload_json == payload_json:
                    return json.loads(existing.payload_json)
                raise PayloadConflictError(
                    f"Payload conflict for delegation release decision: {decision_id}"
                )
            db.add(row)
            db.commit()
        return json.loads(payload_json)

    def get_delegation_release_decision(
        self, decision_id: str
    ) -> dict[str, Any] | None:
        with Session(self.engine) as db:
            row = db.get(DelegationReleaseDecisionRow, decision_id)
        ...
        *,
        ...
        with Session(self.engine) as db:
            ...
            if parent_run_id is not None:
                statement = statement.where(TrustTraceEventRow.parent_run_id == parent_run_id)
            if child_task_id is not None:
                statement = statement.where(TrustTraceEventRow.child_task_id == child_task_id)
            if child_run_id is not None:
                statement = statement.where(TrustTraceEventRow.child_run_id == child_run_id)
            rows = db.scalars(
                statement.order_by(
                    TrustTraceEventRow.occurred_at,
    ...
    def list_trace_event_page(
        ...
        **filters: str | None,
        ...
        """Cursor pagination over the existing Trust trace table.

        The cursor is the last returned stable event id; it is only applied
        after the normal Trust correlation filters, never as an unscoped read.
        """
        if not 1 <= limit <= 1000:
            raise ValueError("trace limit out of range")
        events = self.list_trace_events(limit=1000, **filters)
        if cursor is not None:
            position = next((index for index, event in enumerate(events) if event.id == cursor), None)
            events = [] if position is None else events[position + 1 :]
        selected = tuple(events[:limit])
        return TracePage(
            items=selected,
            next_cursor=selected[-1].id if len(events) > len(selected) and selected else None,
        )
