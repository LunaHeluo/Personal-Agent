...
class McpManager:
    ...
    def session_ready(self, server_id: str) -> bool:
        """Return live transport readiness, independent of persisted status."""
        handle = self._get_handle(server_id)
        return handle.client.session is not None
        ...
        *,
        ...
        async with handle.refresh_lock:
            async with self.lease(server_id) as lease:
                ...
                try:
                    candidate_snapshot = await self._run_protocol_command(
                        lease,
                    ...
                    snapshot = self._activate_discovered_snapshot(
                        server_id, candidate_snapshot
                    )
        ...
        async with handle.refresh_lock:
            ...
            try:
                ...
                async with handle.connect_lock:
                    ...
                    activated = self._activate_discovered_snapshot(
                        server_id, candidate_snapshot
                    )
    ...
    def _activate_discovered_snapshot(
        self,
        server_id: str,
        candidate: Snapshot,
    ) -> Snapshot:
        """Keep the strict active ID stable when live discovery is equivalent."""
        assert self.store is not None
        active = self.store.get_active_snapshot(server_id)
        if (
            active is not None
            and not active.stale
            and active.schema_hash == candidate.schema_hash
        ):
            return active
        return self.store.activate_refreshed_snapshot(server_id, candidate.id)
