...
class PlaywrightNetworkGuard:
    ...
    async def commit_navigation(
        ...
        *,
        ...
        exact_staged_target = final_url in requested_targets
        if (
            not connected_during_navigation
            and not reused_exact_connection
            and not exact_staged_target
        ):
            return False
        # A successful Playwright navigation may be served entirely from its
        # isolated browser cache and therefore create no new proxy connection.
        # In that case authorize only the exact public URL already staged and
        # validated by preflight. Redirects still require connection evidence,
        # so untrusted page content cannot expand the outbound scope.
        targets = (final_url,)
    ...
    async def _handle_connection(
        ...
        try:
            ...
            if self._is_control_endpoint("http", hostname, port):
                ...
                authority = f"{hostname.rstrip('.').casefold()}:{port}"
                if authority not in self._authorized_connection_authorities():
                    raise ValueError("browser_redirect_requires_explicit_navigation")
    ...
    def _authorized_connection_authorities(self) -> set[str]:
        active_sessions = {
            key[1] for key in self._pending_navigations
        } | {
            key[1] for key in self._authorized_targets
        }
        # The shared proxy protocol carries no unforgeable browser-session
        # identity. Fail closed instead of allowing one session to borrow
        # another session's staged authority.
        if len(active_sessions) != 1:
            return set()
        urls = {
            url
            for _sequence, targets, _cursor, _generation in self._pending_navigations.values()
            for url in targets
        }
        urls.update(
            url for _sequence, targets in self._authorized_targets.values() for url in targets
        )
        result = set()
        for url in urls:
            parsed = urlsplit(url)
            ...
            if parsed.hostname:
                result.add(f"{parsed.hostname.rstrip('.').casefold()}:{port}")
        return result
