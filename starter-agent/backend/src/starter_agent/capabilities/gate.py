...
def _is_targetless_browser_read(
    ...
    bounded_wait = (
        action == "read"
        and bool(request.arguments)
        and request.tool_name.casefold().endswith("browser_wait_for")
    )
    return snapshot or bounded_click or bounded_wait
...
class PreToolCallGate:
    ...
    async def evaluate(
        ...
        *,
        ...
        if request.tool_name == "delegate_task" and request.role != "coordinator":
            return self._decision("deny", "delegate_task_coordinator_only", request)
