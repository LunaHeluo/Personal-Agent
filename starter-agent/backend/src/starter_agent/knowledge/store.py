...
class SQLiteKnowledgeStore:
    ...
    def get_document_version(
        ...
        """Return one immutable version only when its document is in scope."""
            ...
            row = db.scalar(
                ...
                .where(
                    *self._scope_filters(scope),
                    DocumentVersionRow.knowledge_base_id
                    == str(knowledge_base_id),
                    DocumentVersionRow.document_id == str(document_id),
                    DocumentVersionRow.id == str(version_id),
                )
            )
        return self._version(row) if row else None

    def find_document_version_by_hash(
        self,
        scope: KnowledgeScope,
        knowledge_base_id: UUID,
        content_sha256: str,
    ) -> DocumentVersion | None:
        """Find an indexed immutable version for idempotent external binding."""
    ...
    def get_chunk(
        ...
        """Resolve an immutable chunk in scope, including non-active versions."""
        ...
        with Session(self.engine) as db:
            row = db.scalar(
                ...
                .where(
                    *self._scope_filters(scope),
                    KnowledgeChunkRow.knowledge_base_id == str(knowledge_base_id),
                    KnowledgeChunkRow.id == str(chunk_id),
                )
            )
