...
_COLLECTION_TITLE = re.compile(
    r"(?:^\s*[\d,]+\+?\s+.*?jobs?(?:\s+in\b|$)|\bbrowse\b.*\bjobs?\b|\bjobs?\s+in\b|"
...
_SOCIAL_HOST = re.compile(
    ...
    r"twitter\.com|x\.com|tiktok\.com)$",
    re.IGNORECASE,
)
_DOCUMENT_SUFFIX = re.compile(r"\.(?:pdf|docx?|xlsx?|pptx?)(?:$|[?#])", re.IGNORECASE)
    ...
    for item in ordered:
        ...
        ranked.append(
            JobCandidate(
                ...
                company_source=str(item.get("company_source") or ""),
                company_confidence=str(item.get("company_confidence") or ""),
...
def assess_job_candidate(
    ...
    *,
    ...
    if _DOCUMENT_SUFFIX.search(path):
        return CandidateAssessment(
            page_kind="social_or_content_page",
            score=0.0,
            reason_codes=("document_not_job_page",),
        )
    ...
    if (
        ...
        or _SOCIAL_HOST.search(hostname)
    ):
        return CandidateAssessment(
            page_kind="social_or_content_page",
            score=0.0,
    ...
    company_confidence = str(item.get("company_confidence") or "")
    if item.get("company") and company_confidence == "high":
        score += 0.12
        reasons.append("company_attribution_high")
    elif item.get("company") and company_confidence == "medium":
        score += 0.07
        reasons.append("company_attribution_medium")
    elif item.get("company"):
        score += 0.05
    elif kind == "organic":
        score -= 0.05
        reasons.append("company_unattributed")
...
def _merge_url_provenance(
    ...
    for item in results:
        ...
        selected = incoming if incoming_kind < current_kind else current
        attribution = preferred_company_attribution(current, incoming)
        selected["company"] = attribution.company
        selected["company_source"] = attribution.source
        selected["company_confidence"] = attribution.confidence
        selected["matched_queries"] = queries
        selected["search_engines"] = engines
        merged[url] = selected
