...
| Skill | `job-research` | `src/starter_agent/skills/job-research/SKILL.md`, loaded by `SkillRegistry` | Skill `version` and `snapshot_hash`; dynamic | Skill load state and dependency state; dynamic | in-process orchestration | Governed search, selected public-page read, JD validation, RAG evidence, analysis, and confirmation before ingestion | Skill Registry state; dynamic | Lightweight metadata first; full definition only after selection | Inherits dependency risks; no direct Tool bypass | Every dependency request is re-evaluated by the Gate | Inherits dependency policy; ingestion needs explicit confirmation | Structured public search/page inputs and bounded results; resume evidence stays within governed RAG flow | Starter Agent maintainer | Skill health endpoint plus dependency state | Disable through the Skill management API |
