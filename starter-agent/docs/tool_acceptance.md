...
## `search_job_description` 手工验收

1. 先使用 `search_jobs_serpapi` 获得至少两条带 URL 的岗位线索；回复“第 2 个”后，确认工具请求使用第 2 条的原始 URL、title 和 company，不猜测或拼接 URL。
2. 成功结果应包含来源 URL、抓取时间、内容哈希、完整性和提取方式；网页内容仅作为不可信 JD 数据处理。
3. 分别验证 robots、403 或动态页面时返回稳定错误，并提示用户打开来源后粘贴 JD；不得以搜索摘要替代 JD。
4. 检查抓取后不会新增 `data/jobs` 记录，也不会自动写入长期记忆；只有用户单独确认保存时才能进入保存流程。
5. 在设置中分别开启和关闭工具治理：开启时查看截断与 token 指标，关闭时结果原样进入上下文。
